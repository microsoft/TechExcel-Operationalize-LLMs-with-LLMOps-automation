---
title: '1. Bootstrapping a New Project'
layout: default
nav_order: 1
...
1. **from a commandline verify the tools are installed and on your path**
...
> **Note**  
> the most reliable way is with a free perosnal github account and using https authentication. Using another type of account (Enterprise) may require additional configuration or troublshootign outside the scope of this guide
...
2. **Define Properties for Bootstrapping**
   ...
   - **GitHub Repo Creation** (related to the new repository to be created)
     - `github_username`: Your GitHub **username**. Ex *placerda*
        > replace the example with yor GitHub username
     ...
     - `github_template_repo`: The project template repository. Ex: *azure/llmops-project-template*. (this value should not be modified)
   ...
   ```
   > **Note**  
   > if you chose to copy and paste these values be sure to modify them as outlined above prior to saving the file
...
3. **Authenticate with Azure and GitHub**
   ...
   > **Note**  
   > the type of login SSH or HTTPS should match the value set in the bootstrap properties file. for best results use ```HTTPS` and ```github_use_ssh="false"```
...
4. **Run the Bootstrap Script**
   ...
   **For PowerShell:**
   ...
   ```
   > **Note**  
   > if you chose to use power you need to validate that you have a policy such as **RemoteSigned** to run the script.
   > ```
